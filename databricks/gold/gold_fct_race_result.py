...
# MAGIC %md
# MAGIC # Gold layer - fct_race_result
...
gold_fct_race_result_df = spark.sql("SELECT * FROM ${table_path.silver_race_result}")
...
gold_fct_race_result_df = gold_fct_race_result_df.select(*business_columns)
...
try:
    table_location = spark.sql(
        f"describe detail {spark.conf.get('table_path.gold_fct_race_result')}"
    ...
    deltaTable.alias("oldData") \
        .merge(
            gold_fct_race_result_df.alias("newData"),
    ...
    spark.sql(
        """
        CREATE OR REPLACE TABLE ${table_path.gold_fct_race_result} (
            race_result_key BIGINT,
            race_key BIGINT,
            driver_key BIGINT,
    ...
    (gold_fct_race_result_df
        ...
        .format("delta")
        .mode("overwrite")
        .saveAsTable(spark.conf.get("table_path.gold_fct_race_result"))
    )
